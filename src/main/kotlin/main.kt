...
fun commissionMastercardAndMaestro(amountDay: Long, amountMonth: Long): String {
    ...
    return if (amountDay in 30_000..7_500_00) "Без комиссии"
    else if (amountDay >= MAX_AMOUNT_DAY) "Сумма превышает дневной лимит!"
    else if (amountMonth >= MAX_AMOUNT_MONTH) "Превышен лимит за месяц!"
    else "Комиссия $sum коп."
}

fun commissionVisaAndMir(amountDay: Long, amountMonth: Long): String {
    val sum = (amountDay / 100 * 0.75)
    return if (amountDay <= MIN_AMOUNT_VISA_MIR) "Комиссия 35 руб."
    else if (amountDay >= MAX_AMOUNT_DAY) "Сумма превышает дневной лимит!"
    else if (amountMonth >= MAX_AMOUNT_MONTH) "Превышен лимит за месяц!"
    else "Комиссия $sum коп."
}
...
fun commissionVkPay(amountDay: Long, amountMonth: Long): String {
    return if (amountDay >= MAX_AMOUNT_DAY_VK_PAY) "Сумма превышает дневной лимит!"
    else if (amountMonth >= MAX_AMOUNT_MONTH_VK_PAY) "Превышен лимит за месяц"
    else "Без комиссии"
}
